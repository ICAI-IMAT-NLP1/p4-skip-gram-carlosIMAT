...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        noise_words: torch.Tensor = torch.multinomial(
            noise_dist, n_samples * batch_size, replacement=True
        )
        ...
        # TODO
        noise_vectors: torch.Tensor = self.out_embed(noise_words).reshape(
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(
        ...
        out_loss = torch.log(
            torch.sigmoid(
                torch.bmm(
                    output_vectors.unsqueeze(1), input_vectors.unsqueeze(2)
                ).squeeze()
            )
        ).sum()
        ...
        noise_loss = torch.log(
            torch.sigmoid(
                torch.bmm(-input_vectors.unsqueeze(1), noise_vectors.transpose(1, 2))
            )
        ).sum(dim=1)
