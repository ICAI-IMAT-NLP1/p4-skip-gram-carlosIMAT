...
def subsample_words(
    ...
    probs = {
        word: 1 - (threshold / freqs[word]) ** 0.5
        for word in freqs
    ...
    train_words = [
        vocab_to_int[word]
        for word in words
        if word not in probs or torch.rand(1).item() > probs[word]
    ]
...
def get_batches(words: List[int], batch_size: int, window_size: int = 5):
    ...
    for idx in range(0, len(words), batch_size):
        batch_end = min(idx + batch_size, len(words))
...
def cosine_similarity(
    ...
    for i, valid_idx in enumerate(valid_examples):
        valid_embedding: torch.Tensor = embeddings[valid_idx].unsqueeze(0)

        dot_product: torch.Tensor = torch.mm(valid_embedding, embeddings.T)
        valid_norm: torch.Tensor = torch.norm(valid_embedding, p=2, dim=1, keepdim=True)
        all_norms: torch.Tensor = torch.norm(embeddings, p=2, dim=1, keepdim=True).T

        similarity: torch.Tensor = dot_product / (valid_norm * all_norms + 1e-8)

        similarities[i] = similarity.squeeze(0)
